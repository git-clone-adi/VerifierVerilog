...
# # ---------------------------------------------------------------------------
# # Structured result — avoids returning loose (bool, str) tuples everywhere
# # ---------------------------------------------------------------------------
...
# # ---------------------------------------------------------------------------
# # Heuristic fallback — for testbenches that don't use [VERIFICATION_SUCCESS]
# # ---------------------------------------------------------------------------
...
#     Priority: explicit tag > fail keywords > unknown.
#     """
#     warnings = []
#     if stderr.strip():
...
#     # Heuristic: look for known error patterns
#     for kw in _FAIL_KEYWORDS:
#         if kw.lower() in stdout.lower():
#             return False, f"Heuristic fail: keyword '{kw}' detected in output.", warnings

#     # No tag and no known failure — ambiguous
#     # Treat as FAIL to be safe (conservative in verification)
#     return (
#         False,
...
# ) -> VerificationResult:
#     """
#     Compiles and simulates a Verilog design + testbench.
#     Uses a temp directory so multiple parallel runs don't collide.

#     Args:
#         design_file:  Path to the RTL/design Verilog file.
#         tb_file:      Path to the testbench Verilog file.
#         timeout_sec:  Hard wall-clock limit for simulation (not compilation).
#         work_dir:     Optional directory for intermediate files. Uses system
#                       temp dir if not provided.

#     Returns:
#         VerificationResult with full context.
#     """
...
#     # Use a temp dir scoped to this call → safe for parallel use
...
#     try:
#         # --- Stage 2: Compilation ------------------------------------------
#         compile_cmd = ["iverilog", "-o", vvp_path,
...
#                 check=True
#             )
#             # iverilog may succeed (rc=0) but still emit warnings on stderr
...
#             # Non-zero exit from vvp itself (runtime crash, not assertion fail)
...
#     finally:
#         # Always clean up if we created the temp dir
